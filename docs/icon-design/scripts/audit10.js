...
let allOk = true;
for (const id of ['mk-g30','mk-g31','mk-g32','mk-g33']) {
  ...
  for (const c of body.matchAll(/ d="([^"]+)"/g)) {
    const toks = c[1].match(/[MLZVH]|-?\d+\.?\d*/g) || []; let i = 0;
    while (i < toks.length) {
      const cmd = toks[i]; i++;
...
console.log(allOk ? 'ALL CHALLENGERS WITHIN 66dp SAFE CIRCLE' : 'SOME EXCEED');
