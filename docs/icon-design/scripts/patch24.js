...
// 1) remove the 3-card block accidentally injected inside card() template
const bad = /<article class="spec" id="s-g34r"[\s\S]*?<\/article>(?=\n  <div class="spec-grid">)/g;
const before = (h.match(bad) || []).length;
h = h.replace(bad, '');
console.log('removed injected blocks inside card() template:', before);

// 2) insert family header + 3 cards at the END of specs section (before its closing </section>)
const marker = '</section>\n\n<section id="showcase"';
if (!h.includes(marker)) { console.error('specs close marker MISS'); process.exit(1); }
...
const block = `
  <div class="family-hd" data-component="family-header"><h3>G34 包络精修变体</h3><span class="caps mono">ENVELOPE VARIANTS · 排布/密度/构图三问</span></div>
  ${cards.join('\n')}
</section>
...
console.log('card() template restored, cards relocated to specs end');
