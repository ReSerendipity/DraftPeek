 ...
 */
@Singleton
class CustomThemeRepositoryImpl @Inject constructor(@ApplicationContext private val context: Context) :
    ...
    override suspend fun importTheme(json: String, fileName: String?): ThemeMetadata? = withContext(Dispatchers.IO) {
    ...
    override suspend fun deleteTheme(themeId: String): Boolean = withContext(Dispatchers.IO) {
    ...
    override suspend fun readThemeJson(themeId: String): String? = withContext(Dispatchers.IO) {
     ...
     */
    private fun isBundledThemeId(id: String): Boolean = id in BUNDLED_THEME_IDS
    ...
    private fun String.replaceRegex(): String = replace(Regex("[^a-z0-9_-]"), "")

    private fun String.ensureJsonExtension(): String = if (endsWith(".json", ignoreCase = true)) this else "$this.json"
}
