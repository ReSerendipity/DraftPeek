 ...
 */
@Immutable
data class DiagnosticItem(val startIndex: Int, val endIndex: Int, val severity: Short, val message: String) {
    companion object {
        /** 提示级别：TODO、尾随空白、风格建议 */
...
object SimpleDiagnosticProvider {
     ...
     * @param out 输出诊断列表
     */
    private fun checkLineIssues(content: String, language: String?, out: MutableList<DiagnosticItem>) {
        ...
        for ((lineIdx, line) in lines.withIndex()) {
            ...
            if (lineLength > 0) {
                ...
                while (wsEnd >= 0 && line[wsEnd].isWhitespace()) {
                    wsEnd--
                }
                val wsStart = wsEnd + 1
    ...
    private fun isCommentLine(trimmed: String, prefixes: List<String>): Boolean =
        prefixes.any { trimmed.startsWith(it) }
     ...
     * @return 行注释前缀列表
     */
    private fun getLineCommentPrefixes(language: String?): List<String> = when (language?.lowercase()) {
