/**
 * 文件功能：Kotlin 语言服务器（kotlin-language-server）客户端存根实现
 *
 * 主要类：[KotlinLspProvider] —— Kotlin 语言的 LSP 客户端实现（待完成）
 *
 * 模块依赖：
 ...
 *
 * 实现路线图（待完成）：
 * 1. 为 Android 捆绑或下载 kotlin-language-server 二进制文件
...
/**
 * Kotlin 语言服务器（kotlin-language-server）客户端存根
 *
 * 此骨架结构设计为在集成 Kotlin 语言服务器二进制文件后填充实际实现。
 *
 * @property context 应用上下文
 */
class KotlinLspProvider @Inject constructor(@ApplicationContext private val context: Context) : LspClient {
    ...
    override suspend fun initialize(rootUri: String): Result<Unit> = Result.failure(
        UnsupportedOperationException("Kotlin LSP is not yet implemented")
    )
