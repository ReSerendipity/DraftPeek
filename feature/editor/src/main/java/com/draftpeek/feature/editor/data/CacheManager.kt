...
class CacheManager @Inject constructor(@ApplicationContext private val context: Context) {
    ...
    private val threadLocalDigest = object : ThreadLocal<java.security.MessageDigest>() {
        override fun initialValue(): java.security.MessageDigest = java.security.MessageDigest.getInstance("SHA-256")
    }
     ...
     */
    @Synchronized
    fun saveUndoState(uri: String, undoManager: UndoManager): Boolean = try {
     ...
     * @return 缓存文件对象
     */
    private fun cacheFileForUri(uri: String): File = File(cacheDir, "undo_${hashUri(uri)}.bin")
