 ...
 *
 * ## 运行前提（CI 与本地一致）
 * 1. 安装 `benchmark` 构建变体的应用（`./gradlew :app:assembleBenchmark`）；
 * 2. 设备已解锁、屏幕常亮、已禁用动画（Macrobenchmark 会自行处理大部分）；
 * 3. 首次启动引导页已走过 —— 否则文件浏览器不会出现，[ensureEditorOpen] 会
 *    以明确的错误信息失败（**不会静默通过**）。
 */
    ...
    return findObject(By.clazz(SORA_EDITOR_CLASS))
        ?: error(
            "在 ${FIND_OBJECT_TIMEOUT_MS}ms 内未找到 $SORA_EDITOR_CLASS。请确认：\n" +
                "1) 安装的是 benchmark 变体（./gradlew :app:assembleBenchmark）；\n" +
                "2) 设备已解锁且在桌面无锁屏遮挡；\n" +
                "3) 首次启动引导页已走完，启动后能进入文件浏览器/编辑器。"
        )
}
...
internal fun UiDevice.ensureEditorOpen() {
    ...
    val newFile = NEW_FILE_LABELS.firstNotNullOfOrNull { label -> findByTextOrDesc(label) } ?: error(
        "既未发现编辑器，也未找到「新建文件」入口（已尝试 ${NEW_FILE_LABELS.joinToString()}）。" +
            "若为全新安装，请先手动走过引导页再运行本基准。"
    )
